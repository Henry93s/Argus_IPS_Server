...
void tsAnalyzingqInit(AnalyzingDataQueue* q, volatile sig_atomic_t* isRunningFlag) {
    ...
    if (pthread_mutex_init(&q->lock, NULL) != 0) {
        perror("AnalyzingDataQueue 뮤텍스 초기화 실패");
        exit(EXIT_FAILURE);
    }
    if (pthread_cond_init(&q->cond, NULL) != 0) {
        perror("AnalyzingDataQueue 조건 변수 초기화 실패");
        exit(EXIT_FAILURE); 
    }
}

// 큐에 새로운 패킷을 추가하는 함수 (생산자 => 캡처 스레드)
void tsAnalyzingqPush(AnalyzingDataQueue* q, AnalyzingData* analyzingData) {
    if (q == NULL || analyzingData == NULL) return;

    // 새 노드 생성
    AnalyzingNode* newNode = (AnalyzingNode*)malloc(sizeof(AnalyzingNode));
    if (newNode == NULL) {
        perror("AnalyzingNode 메모리 할당 실패");
        return;
    }
    newNode->analyzingData = analyzingData;
    newNode->next = NULL;
...
AnalyzingData* tsAnalyzingqPop(AnalyzingDataQueue* q) {
    ...
    */
    ...
    // 큐에서 첫 번째 노드를 꺼냄
    AnalyzingNode* nodeToPop = q->head;
    AnalyzingData* analyzingData = nodeToPop->analyzingData;
    ...
    return analyzingData; // 꺼낸 패킷 데이터 반환
}
...
void tsAnalyzingqDestroy(AnalyzingDataQueue* q) {
    ...
    AnalyzingNode* current = q->head;
    while (current != NULL) {
        AnalyzingNode* to_free_node = current;
        ...
        // if (to_free_packet) {
            ...
            // free(to_free_packet);
        // }
        if (to_free_node->analyzingData) {
            fprintf(stderr, "Warning: A data was left in the queue and will leak memory.\n");
        }
        free(to_free_node);
    }
